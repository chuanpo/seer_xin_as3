package com.robot.core.manager
{
   ...
   public class FitmentManager
   {
      ...
      public static function saveRoomType(info:FitmentInfo, event:Function) : void
      {
         ...
         SocketConnection.addCmdListener(CommandID.SET_FITMENT,function(e:SocketEvent):void
         {
      ...
      {
         SocketConnection.addCmdListener(CommandID.FITMENT_USERING,function(e:SocketEvent):void
         {
            var info:FitmentInfo = null;
            var finfo:FitmentInfo = null;
      ...
      {
         SocketConnection.addCmdListener(CommandID.FITMENT_ALL,function(e:SocketEvent):void
         {
            var info:FitmentInfo = null;
         ...
         if(t == SolidType.SUPER)
         {
            if(MainManager.actorInfo.vip == 1)
            {
               return data.filter(function(item:FitmentInfo, index:int, array:Array):Boolean
               {
                  if(item.unUsedCount > 0)
                  {
                     if(ItemXMLInfo.getIsSuper(item.id))
                     {
                        return true;
                     }
                  }
