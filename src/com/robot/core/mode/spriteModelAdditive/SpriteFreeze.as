package com.robot.core.mode.spriteModelAdditive
{
   ...
   public class SpriteFreeze implements ISpriteModelAdditive
   {
      ...
      {
         var m:ColorMatrixFilter = new ColorMatrixFilter([1,0,0,0,0,0,1,0,0,100,0,0,1,0,100,0,0,0,1,0]);
         if(this.model is BasePeoleModel)
         {
            (this.model as BasePeoleModel).skeleton.getBodyMC().filters = [m];
         }
         else
         {
            this.model.filters = [m];
         }
         this.timer.start();
      ...
      {
         var p:Number = this.timer.currentCount / this.timer.repeatCount;
         this.mc.gotoAndStop(Math.floor(this.mc.totalFrames * p) + 1);
      }
