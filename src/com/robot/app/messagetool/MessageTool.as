package com.robot.app.messagetool
{
   ...
   public class MessageTool
   {
      ...
      {
         TasksManager.getProStatusList(TaskController_38.TASK_ID,function(arr:Array):void
         {
            if(Boolean(arr[1]) && !arr[4])
            {
         ...
         TasksManager.getProStatusList(TaskController_38.TASK_ID,function(arr:Array):void
         {
            if(Boolean(arr[4]) && !arr[8])
            {
               NpcTipDialog.show("在飞船上不要被周围的奇怪现象吓住，找出问题的核心。\n    继续努力哦！",null,NpcTipDialog.IRIS);
            }
         });
         TasksManager.getProStatus(TaskController_38.TASK_ID,8,function(b:Boolean):void
         {
            if(b)
            {
