package com.robot.app.task.control
{
   ...
   public class TaskController_79
   {
      ...
      public static function initLong(m:MovieClip, m1:MovieClip, b:SimpleButton, b1:SimpleButton) : void
      {
         longMc = m;
         _long = m1;
         btn = b;
      ...
      private static function endDh0() : void
      {
         ...
         longMc.stop();
         TasksManager.complete(TASK_ID,1,function(b:Boolean):void
         {
            if(b)
            {
               showPanel();
      ...
      public static function inScean() : void
      {
         if(isMoFangB)
         {
            NpcTipDialog.show(MainManager.actorInfo.nick + "，我也不知道为什么，我就是莫名的紧张……难道是因为马上要和族人见面了？但是，我总感觉O__O!…",function():void
            {
               ...
               {
                  TasksManager.complete(TASK_ID,0,function(b:Boolean):void
                  {
                     if(b)
                     {
                        _long.visible = true;
      ...
      private static function seerOut() : void
      {
         NpcTipDialog.show("……大块头你刚才和那家伙叽里呱啦到底在说些什么？什么塔西什么吼吼的？",function():void
         {
            NpcTipDialog.show("它说这里是它的领地，我们涉足了它的地盘！它是不会就此罢休的……至于我说了什么呢？嘿嘿，我才不告诉你呢！不过，" + MainManager.actorInfo.nick + "你永远都会是我的伙伴！！！！",function():void
            {
               ...
               {
                  TasksManager.complete(TASK_ID,2,function(b:Boolean):void
                  {
                     if(b)
                     {
                        btn.visible = false;
      ...
      public static function playMovie(str:String) : void
      {
         var mclloader:MCLoader = new MCLoader(str,LevelManager.appLevel,1,"正在打开动画");
         mclloader.addEventListener(MCLoadEvent.SUCCESS,onLoadMovie);
         mclloader.doLoad();
      }
