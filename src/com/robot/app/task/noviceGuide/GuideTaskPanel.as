package com.robot.app.task.noviceGuide
{
   ...
   public class GuideTaskPanel extends Sprite
   {
      ...
      {
         var s:String = (e.target as SimpleButton).name;
         var i:int = int(uint(s.substr(3,1)));
         this.createTip(this.tipTxtAry[i - 1],i);
      }
      
      private function createTip(str:String, btnIndex:int) : void
      {
         if(btnIndex == 1 || btnIndex == 4)
         {
            this.tip.x = 350;
            this.tip.y = 93.9;
         }
         else if(btnIndex == 2 || btnIndex == 5)
         {
            this.tip.x = 86;
            this.tip.y = 94;
         }
         else if(btnIndex == 3 || btnIndex == 6)
         {
            this.tip.x = 218;
            this.tip.y = 89;
         }
         var txt:TextField = this.tip["tipTxt"];
         txt.text = str;
