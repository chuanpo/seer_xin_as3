package com.robot.app.task.newNovice
{
   ...
   public class NewNoviceStepOneController
   {
      ...
      public static function start() : void
      {
         ...
         switch(stu)
         {
            case TasksManager.UN_ACCEPT:
               TasksManager.accept(STEP_ID,function(b1:Boolean):void
               {
                  if(b1)
                  {
      ...
      public static function playAnimation(url:String) : void
      {
         _curPath = url;
         _mcLoader = new MCLoader(url,LevelManager.topLevel,1,"正在打开动画");
         _mcLoader.addEventListener(MCLoadEvent.SUCCESS,onLoad);
         _mcLoader.doLoad();
      }
      
      private static function onLoad(e:MCLoadEvent) : void
      {
         _mcLoader.removeEventListener(MCLoadEvent.SUCCESS,onLoad);
         _content = e.getContent() as MovieClip;
      ...
      private static function onPlayComHandler(e:Event) : void
      {
      ...
      private static function onAiCloseHandler(e:MouseEvent) : void
      {
         _content["closeBtn"].removeEventListener(MouseEvent.CLICK,onAiCloseHandler);
         _content.removeEventListener(Event.ENTER_FRAME,onPlayComHandler);
         DisplayUtil.removeForParent(_content);
      ...
      private static function onAppOneOpenHandler(e:Event) : void
      {
         hide1();
         showLoginTipTwo();
      }
      
      private static function onAppOneCloseHandler(e:Event) : void
      {
         hide1();
      ...
      private static function onAppTwoOpenHandler(e:Event) : void
      {
         hide2();
         playAnimation(Ai_2_Str);
      }
      
      private static function onAppTwoCloseHandler(e:Event) : void
      {
         hide2();
         NewNpcDiaDialog.show(DIA_1_A,handler_A,comStep);
      }
