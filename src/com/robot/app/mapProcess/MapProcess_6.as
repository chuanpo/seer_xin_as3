package com.robot.app.mapProcess
{
   ...
   public class MapProcess_6 extends BaseMapProcess
   {
      ...
      {
         var mclloader:MCLoader = new MCLoader("resource/Games/ThruAeroliteGame.swf",LevelManager.appLevel,1,"正在进入磁力光束枪台");
         mclloader.addEventListener(MCLoadEvent.SUCCESS,this.onLoadGameMovie);
         mclloader.doLoad();
      }
      ...
      private function onLoadGameMovie(event:MCLoadEvent) : void
      {
         ...
         mcloader.sharedEvents.addEventListener("False_ThruAeroGame",function(evt:DynamicEvent):void
         {
            mcloader.sharedEvents.removeEventListener("False_ThruAeroGame",arguments.callee);
            var precent:uint = 0;
            switch(uint(evt.paramObject))
            {
               case 1:
                  precent = 40;
                  break;
         ...
         mcloader.sharedEvents.addEventListener("Pass_ThruAeroGame",function(evt:DynamicEvent):void
         {
      ...
      public function onGetComHandler(data:Object) : void
      {
         if(Boolean(data))
         {
            if(!this.exchangeApp)
      ...
      public function gameFun() : void
      {
         ...
         {
            str = "你还没有获取" + TextFormatUtil.getRedTxt("比比鼠发电能源") + "的每日任务呢，" + "快点击右上角的" + TextFormatUtil.getRedTxt("精灵训练营") + "按钮看看吧！";
            Alarm.show(str);
         }
         else
         {
            ...
            if(Boolean(petMode))
            {
               ...
               if(id_str == 13 || id_str == 14 || id_str == 15)
               {
                  this.gameStart();
               }
               else
      ...
      private function fightWithAllision(e:MouseEvent) : void
      {
         ...
         EventManager.addEventListener(PetFightEvent.FIGHT_RESULT,function(evt:PetFightEvent):void
         {
            ...
            TasksManager.complete(TaskController_98.TASK_ID,3,function(b:Boolean):void
            {
            });
