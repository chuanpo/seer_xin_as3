package com.robot.app.systems
{
   ...
   public class MessageAlarm extends BaseBeanController
   {
      ...
      private function onMessage(e:RobotEvent) : void
      {
         show();
      }
   }
...
class MessageAlarmImpl extends Sprite
{
   ...
   private function onClick(e:MouseEvent) : void
   {
      ...
      var id:uint = uint(MessageManager.getFristUnReadID());
      if(id == 0)
      {
         return;
      }
      ...
      else if(id == MessageManager.TEAM_TYPE)
      {
         tinfo = MessageManager.getTeamInformInfo();
         if(Boolean(tinfo))
         {
         }
      }
      else if(id == MessageManager.TEAM_CHAT_TYPE)
      {
      ...
      else
      {
         ...
         TalkPanelManager.showTalkPanel(id);
      }
      if(MessageManager.unReadLength() == 0)
      {
         this.hide();
      ...
      switch(info.type)
      {
         case CommandID.FRIEND_ANSWER:
            if(Boolean(info.accept))
            {
               ...
               sprite.addEventListener(TextEvent.LINK,function(e:TextEvent):void
               {
                  UserInfoController.show(uint(e.text));
