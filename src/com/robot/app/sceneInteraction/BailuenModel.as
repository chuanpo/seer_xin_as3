package com.robot.app.sceneInteraction
{
   ...
   public class BailuenModel extends ActionSpriteModel implements IAimatSprite
   {
      ...
      public function set hp(v:uint) : void
      {
         ...
         this._bloodMc["maskMc"].width = v / 100 * this._bloodMc["viswMc"].width;
         if(v == 0)
         {
            this.setAction(DIE);
         }
      ...
      public function show(p:Point, eff:Boolean) : void
      {
         if(Boolean(this._obj))
         {
            return;
      ...
      public function aimatState(info:AimatInfo) : void
      {
         if(this._currAction != 1)
         {
            return;
         }
         if(atype.indexOf(info.id) == -1)
         {
            return;
         }
         if(Boolean(this._aimatStateManager))
         {
            this._aimatStateManager.execute(info);
         }
      ...
      public function showBox(msg:String, offy:Number = 0) : void
      {
         ...
         this._dialogBox.show(msg,0,-this.height + offy,this);
      }
      ...
      private function setAction(v:int) : void
      {
         if(Boolean(this._obj))
         {
            this._obj.addEventListener(Event.ENTER_FRAME,this.onEnter);
            this._obj.gotoAndPlay(v);
            this._currAction = v;
         }
         stopAutoWalk();
      }
      ...
      private function onLoadPanel(e:MCLoadEvent) : void
      {
         this._load.removeEventListener(MCLoadEvent.SUCCESS,this.onLoadPanel);
         var cla:Class = Utils.getClassFromLoader("UI_BailuenPanel",this._load.loader);
         this._panel = new cla() as Sprite;
         if(Boolean(this._panel))
         {
            LevelManager.topLevel.addChild(this._panel);
            DisplayUtil.align(this._panel,null,AlignType.MIDDLE_CENTER);
