package com.robot.app.popup
{
   ...
   public class FollowPanel
   {
      ...
      public static function show(i:UserInfo) : void
      {
...
class FollowPanelImpl extends Sprite
{
   ...
   public function show(info:UserInfo) : void
   {
      this._info = info;
      var mapName:String = "";
      if(this._info.mapID > MapManager.ID_MAX)
      {
         mapName = this._info.nick + "的基地";
      }
      else
      {
         mapName = MapConfig.getName(this._info.mapID);
      }
      this._txt.htmlText = "你的好友" + this._info.nick + "(" + this._info.userID + ")目前正在" + TextFormatUtil.getRedTxt(mapName) + "上";
